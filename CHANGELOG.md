...
## 2025-10-31 - 批次評估系統與效能優化

### 重大功能更新
- 🚀 **批次 LLM 評估系統**：新增批次評估 API,支援並行處理多筆問答評估
- ⚡ **並發控制機制**：實作信號量模式控制並發請求數,避免 API 過載
- 🔄 **智能重試機制**：自動重試失敗請求,支援指數退避策略
- 💾 **進階快取系統**：10 分鐘 TTL 快取,支援跨頁面使用

### 技術架構改進
- 📡 **請求池管理**：MAX_CONCURRENCY=4 限制同時進行的 API 請求數
- 🛡️ **錯誤處理強化**：
  - 超時保護 (20 秒預設)
  - 自動重試 (2 次,針對 429/5xx 錯誤)
  - 指數退避 (300ms 起始,最大 2000ms)
- 🔐 **安全性提升**：
  - Content-Type 驗證
...
### 效能優化
- ⚡ **並行處理**：批次評估支援 1-4 個並發 worker
- 📊 **進度回報**：即時回報評估進度至 content script
- 🎯 **快取命中率優化**：基於問答內容 hash 的智能快取鍵值

### API 改進
- 📝 **safeFetchJSON**：統一的 HTTP 請求處理函式
  - 可配置超時時間
  - 可配置重試次數
  - 可配置重試條件
- 🔗 **callLLMBatchWithProgress**：支援進度回報的批次評估
- 💡 **inflightCache**：防止相同請求重複發送

### 檔案異動
- `extension/background.js` - 新增批次處理、並發控制、重試邏輯
- `extension/content.js` - 整合批次評估 API(目前未在 UI 開啟)
- `extension/manifest.json` - 維持 Manifest V3 相容性

### 效能指標
- 🚀 單次評估平均時間: ~2-5 秒(視 LLM 回應速度)
- 📊 批次評估吞吐量: ~4 請求/秒(受並發限制)
- 💾 快取命中率: 預期 >80%(相同問答組合)
- ⏱️ 超時保護: 20 秒自動中斷

### 已知限制
- 批次評估 UI 尚未開放(函式已實作)
- 快取僅在 Service Worker 生命週期內有效
- ngrok 端點無身份驗證(待後續改進)

## 2025-09-12 - 文件與指標公式強化

### 新增 / 調整內容
- 📝 **README 大幅重寫**：新增目錄、系統架構圖、資料生命週期、核心資料結構表、KPI 區塊、CSV 欄位說明、Roadmap、進階 FAQ、開發與偵錯指南。
- 🎯 **KPI 區塊改版**：不再直接宣稱最終公式已實作，改為「現況 vs 目標」矩陣，清楚標示尚未實作項（加權解決率、evaluatedPairs 分母、raw 行數、strict unresolved 等）。
- 📊 **指標語意界定**：區分 user 問題筆數（目前分母）與規劃中 raw 總行數 (User+GPT)。
- 📄 **CSV 格式規範**：定義五大匯出檔（含未露出的 PM Template）與欄位語意，為後續 Schema 驗證與自動測試鋪路。
- 🔐 **安全補充**：最小權限策略與外部 LLM 服務風險緩解建議（逾時、重試、Schema 驗證等）。
- 🧠 **資料流程**：加入 DOM 擷取 → 問答配對 → LLM 評估 → KPI 增量更新 → 匯出 的流程與 ASCII 架構圖。

### 澄清 / 修正
- 原 README 中將加權解決率表述為既有行為，現改為標註「尚未實作」。
- 平均正確率目前分母為「全部 user 問題」而非僅已完成評估問答對；文件已標示偏保守並列入 Roadmap。
- 未解決數現行為「全部非完全解決」，未來將拆為 strict 與 extended 兩種。
- 新增對「部分/未知/無回答」如何影響現行統計的說明，避免推導錯誤 KPI。

### 後續規劃（摘錄）
- 加權解決率 (Full + 0.5×Partial)
- 分離 evaluatedPairs 分母
- raw (User+GPT) 雙計數納入
- unresolved_strict 與 unresolved_extended
- Thread 聚合與語意去重
- IndexedDB 評估快取
- Schema 驗證腳本與自動化檢查
- 指標趨勢比較與異常閾值告警
...
### 無程式碼改動
本次僅文件與指標說明調整，`content.js` 現行計算邏輯未改。
