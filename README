...
![version](https://img.shields.io/badge/version-v0.2.0-blue) ![license](https://img.shields.io/badge/license-internal-important)
...
- **單頁記憶體快取**：於同一頁面生命週期內避免重複評估（重新整理後不保留）
- **效能優化**：大幅減少 API 呼叫次數
- **一致性保證**：相同問答組合於同一頁面中結果一致
...
- **錯誤處理**：擷取錯誤並回傳預設值（未知/0%）；目前未實作自動重試
...
```
┌──────────────────────┐      ┌──────────────────────┐
│   Browser Tab (DOM)  │      │  Background Service  │
│  (UserChat Page)     │      │    Worker (MV3)      │
└─────────┬────────────┘      └─────────┬────────────┘
          │ 1. DOM 擷取 (a.textChat-icon.icon)       │
          ▼                                         │
┌──────────────────────┐ 2. 標準化/過濾              │
│   content.js         │────────────────────────────▶ fetch LLM API
│  - Parser            │ 4. 生成 QA Queue            │
│  - Aggregators       │◀──────────┐ 5. 評估結果回傳  │
│  - UI Render         │           │ 6. in-memory 快取 │
└─────────┬────────────┘           │                  │
          │ 3. 建立 maps/sets      │                  │
          ▼                        │                  │
  Data Structures:                 │                  │
...
### 資料生命週期步驟
1. 擷取：掃描符合 selector 的錨點或行記錄
2. 標準化：時間轉換、ID 過濾、排除清單應用
3. 分類：依日期 (YYYY-MM-DD)、小時、userId 聚合
4. 問答配對：依 chatId 或時間窗口配對形成 QA Pair
5. 評估排程：將未評估 pair 放入背景訊息佇列
6. LLM 呼叫：背景 worker 逐一或批次送出
7. 回寫結果：content script 以 runtime message 更新
8. KPI 重算：新評估結果進來時做增量更新
9. 匯出：序列化目前記憶體統計為多 CSV
...
| **狀態** | **項目** | **說明** |
|----------|----------|----------|
| 進行中 | 批次評估 UI | 將背景批次佇列進度條可視化 |
| 計畫中 | PM 匯出按鈕 | 將 `exportPmTemplateCSV` 露出於 UI |
| 計畫中 | 雙語介面 | zh-TW / en-US 切換 |
| 計畫中 | 問答配對策略 v2 | 限制一對一避免多重配對 |
| 計畫中 | IndexedDB 快取 | 跨重新整理保留評估結果 |
| 計畫中 | Schema 驗證 | 對 LLM 回傳格式進行 JSON schema 驗證 |
| 評估中 | 模糊語意去重 | 合併重複提問統計 |
...
**版本**：v0.2.0  
**最後更新**：2025-09-12  
**相容性**：Chrome 88+, Edge 88+  
**授權**：內部使用（若需開源請補充 LICENSE）
